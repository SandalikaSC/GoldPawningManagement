...
class lockerModel extends Database{
 ...
 //to load all lockers.no matter they are reserved or not   
...
//to view details of article inside the locker
public function viewLockerArticle($lockerNo,$articleId){

    $sql1='select a.Article_Id,a.Estimated_Value,a.Karatage,a.Weight,a.Type,a.image,r.Allocate_Id,r.Date,r.Retrive_status,r.Retrieve_Date,r.allocation_fee,r.UserID,r.Keeper_Id,r.appraiser_Id from article a inner join reserves r on a.Article_Id=r.Article_Id where r.Article_Id=? and r.lockerNo=?';
    $this->query($sql1);
